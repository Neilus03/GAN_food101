...
generator = Generator(noise_dim, class_dim, img_channels, img_size_x, img_size_y).to(device)
wandb.watch(generator)
discriminator = ResNetDiscriminator(class_dim).to(device)
...
save_dir = "/home/ndelafuente/CVC/GANs/ACA-GAN/GAN_101/generated_images_food101"
os.makedirs(save_dir, exist_ok=True)

# Training loop
for epoch in range(num_epochs):
    for i, (imgs, labels) in enumerate(train_loader):
        for _ in range(25): #train n times the generator for each time you train the discriminator
            ...
            valid = torch.full((cur_batch_size,1), 0.95, dtype=torch.float32, device=device) #0.95 to avoid the discriminator to be too confident
            fake = torch.full((cur_batch_size, 1), 0.0, dtype=torch.float32, device=device)
            #print(f"valid vector shape: {valid.shape}\n Fake vector shape: {fake.shape}")
            ...
            lambda_aux = 0.75 #lambda for the auxiliary weighted loss
